...
const RestaurantEdit = ({ setModal, current }: IRestaurantEdit) => {
  const [data, setData] = React.useState<IRestaurant | null>(null)
  ...
  async function handleSubmit() {
    ...
    if (image) form.append("image", image.raw)

    setModal("")

    try {
      await axios.put(`${VITE_API_URL}/restaurant/${current}`, form, {
        headers: {
  ...
  const updateValues = () => {
    ...
    address.setInitialValue(data?.address)
    phone.setInitialValue(data?.phone)
    opening.setInitialValue(data?.openingHours)
    type.setInitialValue(data?.cuisineType)

    setDelivery(data?.hasDelivery === "false" ? false : true)
  }

  React.useEffect(() => {
    const getRestaurant = async () => {
      const response = await axios.get(`${VITE_API_URL}/restaurant/${current}`)
